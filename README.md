...
> 3. **Gráfico 3: Cobertura de Stock de Seguridad por Prioridad (Barras Apiladas):** Para evitar la trampa de los promedios (que suelen distorsionarse y ocultar desabastecimientos cuando hay un material muy sobreabastecido), diseñamos un gráfico de barras apiladas. Este compara directamente la cantidad de **artículos físicos reales** en estado de **Stock Suficiente (Verde)** contra los que están **Bajo el Mínimo de Seguridad (Rojo)**, desglosado por prioridad (*CRÍTICA*, *MEDIA*, *NORMAL*). Así, el jefe de compras puede ver la cantidad exacta de ítems en riesgo de forma directa y sin distorsión matemática.
