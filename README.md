...
> 3. **Gráfico 3: Alertas vs. Stock Seguro por Prioridad (Barras Agrupadas con Conteo):** Para evitar la trampa de los promedios y porcentajes abstractos que pueden confundir al usuario, implementamos un gráfico de barras agrupadas (lado a lado). Este muestra de forma **directa y absoluta la cantidad de materiales en Alerta / Quiebre (Rojo)** vs. **Materiales en Stock Seguro (Verde)** para cada nivel de prioridad (*CRÍTICA*, *MEDIA*, *NORMAL*). Además, para garantizar una legibilidad inmediata, desarrollamos un plugin personalizado de etiquetas flotantes que dibuja el número exacto directamente encima de cada barra, permitiendo al usuario leer los datos al instante sin necesidad de pasar el mouse.
