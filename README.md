...
> 3. **Gráfico 3: Salud del Stock por Prioridad / 100% Stacked Bar (% Cobertura):** Para evitar la trampa de los promedios y solucionar la disparidad de volumen (donde categorías como 'Normal' tienen decenas de materiales y 'Crítica' tiene solo 3, haciendo invisibles las barras pequeñas), implementamos un gráfico de barras apiladas al 100%. Muestra visualmente la **proporción porcentual** de *Stock Suficiente (Verde)* vs. *Bajo el Mínimo (Rojo)* para cada prioridad. Además, su tooltip interactivo complementa la estadística revelando el **número real absoluto de materiales** en cada segmento.
